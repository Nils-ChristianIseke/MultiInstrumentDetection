...
class GenSplits:
    ...
    def get_images_in_surgery_folder(self, surgery_folder):
        ...
        image_filepaths=[]
        image_filepaths_l = natsorted(glob.glob(os.path.join(self.data_root, surgery_folder,'image_02', 'images', '*')))
        image_filepaths_r = natsorted(glob.glob(os.path.join(self.data_root, surgery_folder,'image_03', 'images', '*')))
    ...
    def append_lr_to_filenames(indices):
        ...
        for line in indices:
            if search('_02_', line):
                ...
                index = ' '.join((line, 'r'))
            appended_indices.append(index) # CHANGED
    ...
    def prepare_data_paths(self):
        """
        Gets all the image paths, surgery names from the dirs specified by op_dirs
