...
from _base_metric import _BaseMetric#CHANGED
import _timing
...
class HOTA(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            if len(gt_ids_t) == 0: #CHANGED
                gt_ids_t = np.empty((0,), dtype=np.int64) #CHANGED
            if len(tracker_ids_t) == 0: #CHANGED
                tracker_ids_t = np.empty((0,), dtype=np.int64) #CHANGED
